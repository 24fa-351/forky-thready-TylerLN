...
int rand_time() {
  return rand() % 8 + 1;  // Random number between 1 and 8 seconds
}
    ...
    if (pid < 0) {
      ...
      if (ix < num_processes - 1) {
        printf("Process %d (PID: %d) creating Process %d (PID: %d)\n", ix + 1,
               getpid(), ix + 2, pid);  // Use `pid` for the current new process
      }
      // Wait for the child process to finish after printing the creation
      // message
      waitpid(pid, NULL, 0);
    }
  }
...
int main() {
  fork_pattern_two(5);  // Example with 5 processes
  return 0;
}
