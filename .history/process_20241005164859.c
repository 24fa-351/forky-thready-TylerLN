...
void fork_pattern_two(int num_processes) {
  ...
  for (int ix = 0; ix < num_processes; ix++) {
    ...
    if (pid < 0) {
      ...
      printf("Process %d (PID %d) beginning\n", ix + 1, getpid());
      pids[ix] = getpid();  // Store the child's PID
      sleep(rand_time());
      exit(0);
    } else {                       // Parent process
      pids[ix] = getpid();         // Store the child PID
      waitpid(pids[ix], NULL, 0);  // Wait for the child process to finish
      if (ix < num_processes - 1) {
        printf("Process %d (PID: %d) creating Process %d (PID: %d)\n", ix + 1,
               pids[ix], ix + 2, pids[ix + 2]);
      }
    }
  }
