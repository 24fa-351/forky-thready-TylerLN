...
void fork_pattern_one(int num_processes) {
  ...
  for (int ix = 0; ix < num_processes; ix++) {
    ...
    if (pid < 0) {
      ...
      exit(0);         // Child exits after creation
    } else {           // Parent process
      pids[ix] = pid;  // Store child PID
    }
  }
  ...
  // Second loop: Parent handles sleep and child exit
  for (int ix = 0; ix < num_processes; ix++) {
    sleep(sleep_times[ix]);  // Simulate the sleep for each child process
